...
app.config["SQLALCHEMY_DATABASE_URI"] = "postgresql://localhost/sljomaaHW5"
## Provided:
...
class UpdateItemPriority(FlaskForm):
    update=StringField("Enter the new priority for this item", validators=[Required()])
    submit=SubmitField('Update')
...
def update(item):
    form = UpdateItemPriority()
    if form.validate_on_submit():
        priority = form.update.data
        TodoItem.query.filter_by(id=item).first().priority = priority
        item = TodoItem.query.filter_by(id=item).first().description
        db.session.commit()
        flash("Priority of item {} has been updated!".format(str(item)))
...
def delete(lst):
    ...
    db.session.commit()
    flash("Item {} has been deleted.".format(todoList.title))
    return redirect("/all_lists")
    # Replace with code
